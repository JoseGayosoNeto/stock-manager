...
def get_product_metrics():
    ...
    total_cost_price = sum(product.cost_price * product.quantity for product in total_products)
    total_selling_price = sum(product.selling_price * product.quantity for product in total_products)
    total_quantity = sum(product.quantity for product in total_products)
...
def get_sales_metrics():
    ...
    total_sales_value = Outflow.objects.aggregate(total_sales_value=Sum(F('quantity') * F('selling_price')))['total_sales_value'] or 0
    
    total_sales_cost = Outflow.objects.aggregate(total_sales_cost=Sum(F('quantity') * F('cost_price')))['total_sales_cost'] or 0
